    ...
    const fetchData = async () => {
      ...
      try {
        ...
        for await (const l of letters) {
          l.recipients = await fetchRecipients(l.id);
        }
        setLetters(letters);
      } catch (err) {
    ...
    <div className="bg-gray-100 min-h-screen py-6">
      <div className="max-w-lg mx-auto bg-white shadow-md rounded-lg overflow-hidden">
        ...
        <header className="flex justify-between items-center bg-blue-100 p-5 border-b border-gray-200">
          {/* Profile Loading Skeleton & Spinner */}
          {loadingUser ? (
        ...
        <main className="p-6">
          <section className="mt-8">
            ...
            {/* Letters Loading Spinner & Skeleton */}
            {loadingLetters ? (
              <div className="flex flex-col items-center space-y-4">
                <div className="w-10 h-10 border-4 border-gray-300 border-t-blue-500 rounded-full animate-spin"></div>
                <p className="text-gray-500">Fetching letters...</p>
                {[...Array(3)].map((_, i) => (
                  <div key={i} className="p-4 bg-gray-200 rounded-lg w-full h-16 animate-pulse"></div>
                ))}
              </div>
            ) : letters.length > 0 ? (
