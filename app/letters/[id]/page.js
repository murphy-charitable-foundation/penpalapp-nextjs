  ...
  try {
    ...
    if (!draftSnapshot.empty) {
      ...
      const draftData = {
        id: draftDoc.id,
        ...draftDoc.data(),
        created_at: draftDoc.data().created_at?.toDate?.() || draftDoc.data().created_at,
        updated_at: draftDoc.data().updated_at?.toDate?.() || draftDoc.data().updated_at,
      };
...
export default function Page({ params }) {
  ...
  const handleUpdateMessage = async () => {
    ...
    try {
      ...
      if (error.code === "permission-denied") {
        alert("Permission denied. Please check your access rights to this conversation.");
          ...
          const messagesWithSenderInfo = await Promise.all(
            sortedMessages.map(async (message) => {
              if (message.sent_by?.id !== currentUser.uid) {
                const recipient = fetchedRecipients.find((r) => r.id === message.sent_by?.id);
                if (recipient) {
  ...
  const getSenderLocation = (message) => {
    const isSenderUser =
      message.sent_by?.id === user?.uid || message.sent_by?.path === `users/${user?.uid}`;
  ...
  return (
    <PageBackground className="bg-gray-100 h-screen flex flex-col overflow-hidden">
      ...
      >
          ...
          {allMessages.map((message, index) => {
            ...
            return (
              <div key={messageId}>
                ...
                >
                  <div className="px-4 py-3" onClick={() => selectMessage(messageId)}>
                    <div className="flex items-center">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center">
                          ...
                          {location && <span className="text-black ml-2 text-sm">{location}</span>}
                        </div>
        ...
        {showConfirmReportPopup && <ConfirmReportPopup setShowPopup={setShowConfirmReportPopup} />}
      </PageContainer>
    </PageBackground>
  );
}
