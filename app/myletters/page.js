      ...
      <div className="fixed inset-0 bg-gray-900 bg-opacity-70 z-50 flex justify-center items-center">
        <div className="bg-white p-8 rounded-3xl shadow-2xl max-w-2xl w-full m-4 space-y-6 bg-gradient-to-br from-white to-gray-50">
          <div className="flex justify-between items-center">
            ...
            <button
              onClick={onClose}
              className="rounded-full text-gray-500 hover:text-gray-800 focus:outline-none transition duration-150 ease-in-out"
            >
              <svg
          ...
          <div className="flex justify-end gap-4 mt-4">
            <button
              onClick={onClose}
              className="py-2 px-6 text-sm font-medium text-white bg-blue-600 rounded-full hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-opacity-50 transition ease-in-out duration-300"
            >
              Close
            </button>
          </div>
        ...
        <main className="p-12">
          {letters.length > 0 ? (
            letters.map((letter) => (
              <div
                key={letter.id}
                className="flex items-center p-4 mb-3 rounded-lg hover:bg-gray-200 hover:scale-105 transition-all duration-300"
            ...
            >
              <div className="space-y-4">
                <h3 className="text-lg font-semibold text-gray-800">
                  Letter from {activeLetter.senderId}
                </h3>
                <p className="text-gray-600">{activeLetter.content}</p>
                <p className="text-sm text-gray-500">
                  Received: {activeLetter.received}
                </p>
              </div>
              {!isReplying ? (
                <button
                  onClick={() => setIsReplying(true)}
                  className="mt-4 py-2 px-6 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition ease-in-out duration-300 focus:outline-none focus:ring-2 focus:ring-blue-300 focus:ring-opacity-50"
                >
                  Reply
                </button>
              ) : (
